...
PrgURLEnable(PrgUrlTest, UrlPrgIsCompressed, selPrgChoice, PrgChoicePaths, selPrgChoiceTimer, PrgLnkInf, PrgUrl, PrgVer, PrgVerOld, UpdturlHwnd, 1)
...
if Util_VersionCompare(PrgVer, PrgVerOld)
	...
	if (retVal < 0) ;Did the user type?
		...
		if (retVal)
			...
			GuiControl, PrgLnchOpt:, RnPrgLnch, &Test Run Prg
				...
				GuiControl, PrgLnchOpt:, MkShortcut, % ChgShortcutVar
				GuiControl, PrgLnchOpt: Enable, RnPrgLnch
				PrgURLEnable(PrgUrlTest, UrlPrgIsCompressed, selPrgChoice, PrgChoicePaths, selPrgChoiceTimer, PrgLnkInf, PrgUrl, PrgVer, PrgVerOld, UpdturlHwnd)
...
PrgURLEnable(ByRef PrgUrlTest, ByRef UrlPrgIsCompressed, selPrgChoice, ByRef PrgChoicePaths, ByRef selPrgChoiceTimer, PrgLnkInf, PrgUrl, ByRef PrgVer, ByRef PrgVerOld, UpdturlHwnd, UrlDisableGui := 0)
{
...
;msgbox % UrlDisableGui "N" selPrgChoice "NN" PrgUrl[selPrgChoice]  "NNN" PrgLnkInf[selPrgChoice]
	if (temp)
	{
		...
		else
		{
			...
			FileGetVersion, PrgVer, % PrgChoicePaths[selPrgChoice]
			if (ErrorLevel)
			MsgBox, 8192, , % "Problem with file retrieving version for " PrgChoicePaths[selPrgChoice]

			UrlPrgIsCompressed := ChkURLPrgExe(temp)
...
NewThreadforDownload: ;Timer!
	...
	DownloadFile(LC_UrlEncode(PrgUrl[selPrgChoice]), fTemp, updateStatus)
		...
		IniRead, temp, %PrgLnchIni%, General, PrgLaunchAfterDL
			...
			if !(temp)
			{
			MsgBox, 8195, , Launch the newly downloaded Prg to test it? `nIf replying 'Yes', PrgLnch will require the launced Prg to be closed before continuing any further.`n`nReply:`nYes: Launch (Warn like this next time)`nNo: Do not launch (Warn like this next time) `nCancel: Do not launch (This will not show again): `n
			IfMsgBox, Yes
